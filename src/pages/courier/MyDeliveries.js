...
export default function MyDeliveries() {
    ...
    const fetchMyDeliveries = async () => {
        try {
            const response = await myAxios.get('/api/courier/my-deliveries');
            setOrders(response.data);
            setLoading(false);
        } catch (err) {
            console.error('Error fetching my deliveries:', err);
            setError(`Failed to fetch my deliveries: ${err.message}`);
            setLoading(false);
        }
    };
    ...
    const handleMarkAsDelivered = async (orderId) => {
        try {
            await myAxios.post(`/api/courier/mark-delivered/${orderId}`);
            ...
            <div className="table-responsive">
                <table className="table table-striped">
                    ...
                    <tbody>
                        ...
                        {orders.length === 0 && (
                            <tr>
                                <td colSpan="6" className="text-center">Nincsenek saját kiszállítások</td>
                            </tr>
                        )}
                    </tbody>
                </table>
